...
for i in $(echo $DISCO_MOUNT_EXPORT_MAP | sed "s/,/ /g"); do
  ...
  for dflist in $(df -h | grep $REMOTE_IP:$REMOTE_EXPORT ); do
    ...
    # Add the export
    echo "$DISCO_LOCAL_MOUNTPOINT   $EXPORT_CIDR(rw,wdelay,no_root_squash,no_subtree_check,fsid=$FSID,sec=sys,rw,secure,no_root_squash,no_all_squash,crossmnt)" >>/etc/exports
    FSID=$((FSID + 10))
